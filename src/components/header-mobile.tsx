...
interface SelectButtonMobileProps {
    background?: string;
    color?: string;
}

const SelectButtonMobile = styled.button<SelectButtonMobileProps>({
    fontSize: "14px",
    padding: "0.3em 0.6em",
...
const HeaderMobile: React.FC<HeaderMobileProps> = ({ position, popupHandler }) => {
    const [menuIcon, setMenuIcon] = useState<"open" | "close">("close");
    ...
    const preislisteHandler = (e: "open" | "close"): void => {
        if (e === "close") {
            setMenuIcon(e);
            setSubMenu(false);
            setPreisliste(false);
            ...
            >
                <div css={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                    <div css={{ width: "100px", padding: "0.7em 0" }}>
                        <StaticImage
                            src={`../images/bei_lisa_square.png`}
                    ...
                    <motion.div animate={menuIcon === "open" ? { background: dark, color: beige } : { background: beige, color: dark }} onClick={() => subMenuHandler()} css={{ background: beige, borderRadius: "50%", width: "36px", height: "36px", position: "relative" }}>
